...
class Organization < ActiveRecord::Base
  ...
  def ignore_fields
    [ :id,
      ...
      :barter_status,
      :discounts_navigation_title,
      :photos_navigation_title,
      :afishas_navigation_title,
      :reviews_navigation_title,
      :address_navigation_title,
      :csv_id,
      :show_custom_balloon_icon,
      :stat
    ]
  end
  ...
  def self.available_suborganization_kinds
    Dir[Rails.root.join('app/models/suborganizations/*.rb')].map { |f| f.split('/').last.gsub '.rb', '' } - ['shop']
  end
  ...
  def map_image_name(slug, image_type = 'default')
    return resized_image_url(primary_organization.try(:logotype_url) || logotype_url, 70, 70) if show_custom_balloon_icon || primary_organization.try(:show_custom_balloon_icon)
